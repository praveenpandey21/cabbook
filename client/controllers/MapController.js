...
$(function() {
 ...
 $scope.initMap =  function () {
          ...
          infoWindow = new google.maps.InfoWindow;
                ...
                socket.on('RemoveDriver', function(driv){
                //  console.log(driv);
                  if (cabmarkers[driv.id] != undefined) {
                    cabmarkers[driv.id].setMap(null);
                     }
                  });
                }
              ...
              function geocodeLatLng(geocoder, map, infowindow, pos) {
               ...
               var marker = new google.maps.Marker({
                position: latlng,
                 map: map,
                draggable :true,
                icon: 'public/images/1494844557_icon-man.png'
               });
...
//  console.log(data);
  if(!data.Status){
    ...
    alert('You can not book the same cab');
  }else{
  //  console.log(data);
    var customer = $cookies.getObject('authUser');
    ...
    $scope.Booking.Fare =$scope.fare; //totalAmount;
    $scope.Booking.Customer = customer.currentUser.userInfo;
    $scope.Booking.Distance =$scope.dist; //distance;
    $scope.Booking.BookingType = 'Current';
    $scope.Booking.cabrcarType = $scope.cartype;
    $scope.Booking.BookingStatus = 'Completed';
    $scope.Booking.PickupDate = new Date().getDate() + '-'+new Date().getMonth()+'-'+new Date().getFullYear();
    $scope.Booking.PickupTime = document.getElementById('timepicker').value;//new Date().getHours()+':'+new Date().getMinutes();
...
$(function(){
  ...
  document.getElementById('modalstart').innerHTML = document.getElementById('origin').value;
  document.getElementById('modaldest').innerHTML =document.getElementById('destination').value;
  //$('#myModal').modal();
  $scope.Booking.PickupLocation =  document.getElementById('origin').value;
  $scope.Booking.DestinationLocation = document.getElementById('destination').value;
  $scope.Booking.Fare =$scope.fare; //totalAmount;
